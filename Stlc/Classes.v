...
Class SubstTheory (exp : nat -> Set) (u : nat -> Set) (uvar : forall {n}, atom -> u n) `{H: Subst exp u} := {
...
(*
Hint Rewrite (fun {e} {u} {v} `{K:SubstTheory e u v} => @close_open e u v _ _) 
  using solve [auto; try typeclasses eauto] : syntax.

Hint Rewrite (fun {e} {u} {v} `{K:SubstTheory e u v} => @open_close e u v _ _) 
  using solve [auto; try typeclasses eauto] : syntax.

Hint Rewrite (fun {e} {u} {v} `{K:SubstTheory e u v} => @subst_open_var e u v _ _) 
  using solve [auto; try typeclasses eauto] : syntax.
*)
